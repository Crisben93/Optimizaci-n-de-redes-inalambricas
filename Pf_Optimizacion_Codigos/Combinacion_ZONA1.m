...
%% Cordenadas
xCmin = round(min(X_1(:,1)));
xCmax = round(max(X_1(:,1)));
yCmin = round(min(X_1(:,2)));
yCmax = round(max(X_1(:,2)));
...
CCC=0;
% Coordenadas Ap
for yAP = yCmin:5:yCmax % No se considera el borde del plano
    for xAP = xCmin:5:xCmax
        x1 = xAP; % Eje x
        y1 = yAP; % Eje y
        p1 = [x1,y1]; % Punto Ap
        ...
        wp = 11.5; % Wp = factor de perdidas por muro [En cemento normal 8- 12 dB]
         ...
         Sum_p = 0;
         T = size(X_1); 
        for i = 1:T(1) % No se considera el borde del plano
            CCC = CCC + 1;
            xp = round(X_1(i,1));
            yp = round(X_1(i,2)); 
        ...
        contador = contador + 1;
        suma(contador)= Sum_p;
        cordenada(1,contador)=yAP;
        cordenada(2,contador)=xAP;
    end
end
[m,U]=max(suma);
cordenada(1,U)
...
plot(cordenada(2,U),cordenada(1,U),'o','Color','black','LineWidth',5)
hold on
legend('Usuarios AP 1','Usuarios AP 2','Usuarios AP 3','Usuarios AP 4','Usuarios AP 5','Ubicacion de AP','AP-Convinado-1')
title('Comparacion entre Metodo: K-means y Convinado','interpreter','latex','fontsize',24')